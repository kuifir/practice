...
public final class LazySingletonWithSynchronizedDoubleCheck {
    private static LazySingletonWithSynchronizedDoubleCheck instance= null;//不实例化
    private LazySingletonWithSynchronizedDoubleCheck(){}//构造函数
    public static LazySingletonWithSynchronizedDoubleCheck getInstance(){//加同步锁，通过该函数向整个系统提供实例
        if(null == instance){//第一次判断，当instance为null时，则实例化对象，否则直接返回对象
            synchronized (LazySingletonWithSynchronizedDoubleCheck.class){//同步锁
                if(null == instance){//第二次判断
                    instance = new LazySingletonWithSynchronizedDoubleCheck();//实例化对象
                }
            }
        }
