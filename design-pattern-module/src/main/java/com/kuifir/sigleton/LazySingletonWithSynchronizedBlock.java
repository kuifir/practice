...
public final class LazySingletonWithSynchronizedBlock {
    private static LazySingletonWithSynchronizedBlock instance= null;//不实例化
    private LazySingletonWithSynchronizedBlock(){}//构造函数
    public static synchronized LazySingletonWithSynchronizedBlock getInstance(){//加同步锁，通过该函数向整个系统提供实例
        if(null == instance){//当instance为null时，则实例化对象，否则直接返回对象
            instance = new LazySingletonWithSynchronizedBlock();//实例化对象
        }
